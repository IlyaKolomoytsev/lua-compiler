...
def process_lua_files(directory):
    # Формируем путь с расширением .lua
    lua_files_pattern = os.path.join(directory, '**', '*.lua')

    # Ищем все файлы, которые соответствуют шаблону
    lua_files = glob.glob(lua_files_pattern, recursive=True)
...
if __name__ == "__main__":
    files = process_lua_files(BASE_RUN_DIRECTORY)
